...
function initPageAuthority() {
    $.ajax({
        type: "POST",
        url: "WarehouseConfigIndication.aspx/AuthorityControl",
        data: "",
...
function LoadWareHouseName(mOrganizationId) {  //括号内应该为空   网页之上下拉菜单的加载
    $.ajax({
        type: "POST",
...
function save() {   //窗口的添加的保存
    ...
    mWarehousenameId = $('#name').combobox('getValue');
    ...
    if (mWarehousenameId == "" || mVariableid == "")  //|| mCreator == ""
        ...
        if (IsAdd == true) {
            ...
            mdata = "{mWarehousingtype:'" + mWarehousingtype + "',mWarehousenameId:'" + mWarehousenameId + "',mVariableid:'" + mVariableid + "',mSpecies:'" + mSpecies + "',mDatabasename:'" + mDatabasename + "',mDatatablename:'" + mDatatablename + "',mMultiple:'" + mMultiple +
