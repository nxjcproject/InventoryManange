            ...
            if (warehouseNameNew == "全部")
            {
                for (int i = 0; i < idTable.Rows.Count; i++)
                {
                    ...
                    Id = Convert.ToString(idTable.Rows[i][2]);
                     ...
                     outputQuantity = GetInputQuantity(startTimeNew, getBenchmarksInformationTable, organizationIdNew, getOutputWarehouseTable);//出库量
        ...
        private static Decimal GetInputQuantity(DateTime startTime, DataTable BenchmarksInformation, string organizationID, DataTable InputWarehouse)
        {
            ...
            for (int i = 0; i < tableRowsCount; i++)
            {
                ...
                if (dataTableName == "material_MaterialChangeContrast")
                {
                    ...
                    DataTable table = GetMaterialChangeDataTable(organizationID, "cementmill", BenchmarksTime, startTime);
                ...
                else if (dataTableName == "VWB_WeightNYGL")
                {
                    ...
                    else
                    {
                        ...
                        ISqlServerDataFactory dataFactoryNew = new SqlServerDataFactory(connectionString);
                        string mySql = @"select sum(Value) as InputSigle from [dbo].[VWB_WeightNYGL] 
                                       where @BenchmarksTime<StartTime and StartTime<@startTime and OrganizationID=@OrganizationID and VariableId=@VariableId and [VariableSpecs]=@specsName
                                      ";
                        SqlParameter[] myParameter = { new SqlParameter("@startTime", startTime), new SqlParameter("@BenchmarksTime", BenchmarksTime), new SqlParameter("@OrganizationID", organizationID), new SqlParameter("@VariableId", variableIdName), new SqlParameter("@specsName", specsName) };
                        DataTable table = dataFactoryNew.Query(mySql, myParameter);
                        try
                        {
                            inputSigleNygl = Convert.ToDecimal(table.Rows[0][0]);
                ...
                else if (dataTableName == "HistoryDCSIncrement")
                {
                    ...
                    SqlParameter[] myParameter = { new SqlParameter("@startTime", startTime), new SqlParameter("@BenchmarksTime", BenchmarksTime)};
